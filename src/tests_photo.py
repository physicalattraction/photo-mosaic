...
class PhotoTestCase(TestCase):
    ...
    def test_that_uniform_image_returns_that_color(self):
        for color in [(0, 0, 0), (255, 255, 255), (255, 0, 0)]:
            photo = Photo(img=Image.new(size=(100, 50), color=color, mode='RGB'))
    ...
    def test_that_real_photo_returns_correct_color(self):
        photo = Photo(filepath=Path.to_photo('wolf_low_res'))
    ...
    def test_that_two_photos_are_not_equal_if_not_equal_in_size(self):
        photo_1 = Photo(img=Image.new(size=(10, 10), color=(0, 0, 0), mode='RGB'))
        photo_2 = Photo(img=Image.new(size=(10, 10), color=(0, 0, 0), mode='RGB'))
        self.assertEqual(photo_1, photo_2)
        photo_3 = Photo(img=photo_1.resize((5, 5)))
